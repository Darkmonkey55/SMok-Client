...
import net.minecraft.client.me.sleepyfish.smok.utils.render.shader.ShaderUtils;
...
public class RoundedUtils {
    ...
    private static final ShaderUtils roundedGradientShader = new ShaderUtils(FileUtils.path + "/shaders/roundedRectGradient.frag");
    private static final ShaderUtils roundedTexturedShader = new ShaderUtils(FileUtils.path + "/shaders/roundRectTextured.frag");
    public static ShaderUtils roundedOutlineShader = new ShaderUtils(FileUtils.path + "/shaders/roundRectOutline.frag");
    public static ShaderUtils roundedShader = new ShaderUtils(FileUtils.path + "/shaders/roundedRect.frag");

    public static void drawGradientRoundLR(float x, float y, float width, float height, float radius, Color color1, Color color2) {
        RoundedUtils.drawGradientRound(x, y, width, height, radius, color1, color2, color2, color1);
    }
    ...
    private static void setupRoundedRectUniforms(float x, float y, float width, float height, float radius, ShaderUtils textShader) {
        ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft());
        textShader.setUniformf("location", x * sr.getScaleFactor(),
                (Minecraft.getMinecraft().displayHeight - (height * sr.getScaleFactor())) - (y * sr.getScaleFactor()));
        textShader.setUniformf("rectSize", width * sr.getScaleFactor(), height * sr.getScaleFactor());
        textShader.setUniformf("radius", radius * sr.getScaleFactor());
    }
